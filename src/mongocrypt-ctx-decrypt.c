                                    ...
                                    mongocrypt_status_t *status)
{
   ...
   /* look up the key */
   if (!_mongocrypt_key_broker_decrypted_key_material_by_id (kb, &ciphertext.key_id, &key_material)) {
      /* We allow partial decryption, so this is not an error. */
      /* TODO: either log here and pass a mongocrypt_ctx_t instead of a key broker
       * or log inside the key broker (and add a pointer to mongocrypt_ctx_t there)
       *
